...
impl WorkerManager {
    ...
    pub async fn get_dp_info(url: &str, api_key: Option<&str>) -> Result<DpInfo, String> {
        ...
        // Try to get model_id with fallbacks
        let model_id = info
            .model_id
            .or_else(|| {
                info.model_path
        ...
        for base_url in base_urls {
            match Self::get_dp_info(base_url, api_key).await {
                Ok(dp_info) => {
                    ...
                    // Generate DP-aware URLs using legacy format: url@rank
    ...
    pub async fn initialize_workers(
        ...
        match &config.mode {
            ...
            RoutingMode::OpenAI { .. } => {
                // OpenAI mode doesn't initialize workers - they're managed by OpenAI
    ...
    fn remove_single_worker(worker_url: &str, context: &AppContext) -> Result<String, String> {
        ...
        // Update cache-aware policy with remaining workers
    ...
    fn remove_dp_aware_workers(worker_url: &str, context: &AppContext) -> Result<String, String> {
        ...
        // Update cache-aware policies for affected models
    ...
    fn parse_server_info(json: Value) -> Result<ServerInfo, String> {
        // Manual parsing for compatibility with different response formats
        // We always use manual parsing because the struct has all Optional fields
        // which means serde will always succeed even with incompatible JSON
        Ok(ServerInfo {
            model_id: json
                .get("model_id")
                .and_then(|v| v.as_str())
